// components/ProductGridBestpreis.tsx
"use client";
...
const ProductGridBestpreis = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        console.log('🔍 Fetching Bestpreis products...');
        ...
        const productQuery = `
          *[_type == "product" 
            && status == "new" 
            && stock > 0 
            && references(*[_type == "brand" && slug.current == "bestpreis"]._id)
        ...
        console.log(`✅ Found ${response?.length || 0} Bestpreis products`);
        
        setProducts(response || []);
        
      } catch (error) {
        console.error("❌ Error fetching Bestpreis products:", error);
        setError(error instanceof Error ? error.message : "Fehler beim Laden der Bestpreis Produkte");
        setProducts([]);
      } finally {
        setLoading(false);
      }
      ...
      <Container className="flex flex-col lg:px-0 my-8 sm:my-10">
        <div className="text-center mb-6 sm:mb-10">
          ...
          <p className="text-base sm:text-lg text-gray-700 mt-2 sm:mt-3">
            Entdecken Sie die neuesten Top-Angebote zum Bestpreis
          </p>
        ...
        <div className="text-center py-14 sm:py-16 bg-red-50/60 rounded-xl mt-4 sm:mt-10 border-2 border-red-200/50">
          ...
          <p className="text-base sm:text-lg text-red-600 max-w-md mx-auto">
            {error}
          </p>
          <button
      ...
      {loading ? (
        ...
        <div className="text-center py-14 sm:py-16 bg-amber-50/60 rounded-xl mt-4 sm:mt-10 border-2 border-amber-200/50">
          ...
          <h3 className="text-xl sm:text-2xl font-bold text-gray-800 mb-2">
            Keine Bestpreis Produkte verfügbar
          </h3>
          <p className="text-base sm:text-lg text-gray-600 max-w-md mx-auto">
            Es tut uns leid, aber es gibt derzeit keine neuen Produkte zum{" "}
            <span className="font-bold text-gray-900">Bestpreis</span>
          </p>
          <p className="text-sm sm:text-base text-gray-500 mt-3">
            Schauen Sie später wieder vorbei – wir haben regelmäßig neue Top-Angebote!
          </p>
...
export default ProductGridBestpreis;