      ...
      <div className="pb-2">
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-3 text-sm border-2 border-[#E8E3D8] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#D4A853] focus:border-[#D4A853] transition-all bg-white/80 focus:bg-white placeholder:text-[#8A7A6A]"
          />
          {searchTerm && (
            <button
        ...
        {selectedBrand && selectedBrandData && (
          <div className="flex items-center gap-3 mb-3 px-3 py-2 bg-[#F5F0E8] rounded-xl border border-[#D4A853]">
            ...
            <button
              ...
              className="text-xs text-[#8A7A6A] hover:text-[#B8923A] transition-colors"
              type="button"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        <div className="space-y-2">
          {sortedBrands?.length === 0 ? (
            <div className="text-center py-6 text-sm text-[#8A7A6A]">
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isSelected
                      ? "bg-[#F5F0E8] border border-[#D4A853]"
                      : "hover:bg-[#F5F0E8] border border-transparent"
                  )}
                >
                  <div className="flex-shrink-0 w-4 h-4 rounded-sm border-2 border-[#E8E3D8] flex items-center justify-center transition-colors">
                    {isSelected && <div className="w-2.5 h-2.5 rounded-sm bg-[#D4A853]" />}
                  </div>
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    {brand?.logo && (
                      <Image
                        ...
                        width={48}
                        height={48}
                        className="w-12 h-12 rounded-full object-contain bg-white border border-[#E8E3D8] p-1 flex-shrink-0"
                      />
                    )}
                    <span
                      className={cn(
                        "text-sm flex-1",
                        isSelected ? "font-semibold text-[#1a1a1a]" : "font-normal text-[#1a1a1a]"
                      )}
                    >
                      {brand?.name || "Unbenannte Marke"}
                    </span>
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {brand?.featured && <Star className="w-4 h-4 text-[#D4A853] fill-[#D4A853]" />}
                      {brand?.marketLocation && <MapPin className="w-4 h-4 text-[#B8923A]" />}
                    </div>
                  </div>
                </div>
              );
    ...
    <div className="w-full bg-white rounded-xl border border-[#E8E3D8] shadow-sm overflow-hidden mb-4">
      <div className="bg-[#F8F6F2] border-b border-[#E8E3D8] px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-[#1a1a1a]">Marken</h3>
            {typedBrands && typedBrands.length > 0 && (
              <span className="text-xs text-[#8A7A6A] bg-white/70 px-2 py-0.5 rounded-full">
                {typedBrands.length}
              </span>
            )}
          </div>
        ...
        {selectedBrand && selectedBrandData && (
          <div className="mt-1.5 flex items-center gap-2">
            ...
            <span className="inline-flex items-center gap-2 px-3 py-1 bg-[#F5F0E8] text-[#1a1a1a] text-sm font-medium rounded-full border border-[#D4A853]">
              {selectedBrandData?.logo && (
                <Image
                  src={typeof selectedBrandData.logo === "string" ? selectedBrandData.logo : urlFor(selectedBrandData.logo).url()}
                  alt={selectedBrandData.name || "Brand"}
      ...
      <div className="px-3 pt-3 pb-1">
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#8A7A6A]" />
          <input
            type="text"
            placeholder="Marken durchsuchen..."
            value={searchTerm}
            onChange={handleSearchChange}
            className="w-full pl-8 pr-8 py-2 text-sm border-2 border-[#E8E3D8] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#D4A853] focus:border-[#D4A853] transition-all bg-white/80 focus:bg-white placeholder:text-[#8A7A6A]"
          />
          {searchTerm && (
            <button
              onClick={handleClearSearch}
      ...
      <div className="px-2 pb-3 max-h-[280px] overflow-y-auto custom-scrollbar">
        <div className="space-y-1">
          {sortedBrands?.length === 0 ? (
            <div className="text-center py-3 text-sm text-[#8A7A6A]">
              {searchTerm ? "Keine Marken gefunden" : "Keine Marken verfügbar"}
            </div>
          ) : (
            sortedBrands.map((brand: BrandItem) => {
